...
namespace rt
{
    float Cylinder::hit(const math::Ray &ray) const
    {
        ...
        const float a =
            static_cast<float>(ray.direction.dot(ray.direction) - pow(ray.direction.dot(_direction.normalize()), 2));
        const float b = 2.0f *
            (ray.direction.dot(oc) - (ray.direction.dot(_direction.normalize()) * oc.dot(_direction.normalize())));
        const float c = static_cast<float>(oc.dot(oc) - pow(oc.dot(_direction.normalize()), 2) - _radius * _radius);
